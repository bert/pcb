...
via_hole_cb(GtkWidget *widget, SizesDialog *sd)
	{
	gdouble	max, min, hole_size;
	gint	min_via_size;

	gtk_spin_button_get_range(GTK_SPIN_BUTTON(sd->via_size_spin_button),
				&min, &max);
	hole_size = gtk_spin_button_get_value(GTK_SPIN_BUTTON(widget));
	min_via_size = TO_PCB_UNITS(hole_size) + MIN_PINORVIACOPPER;
	if (min_via_size > min)
		gtk_spin_button_set_range(GTK_SPIN_BUTTON(sd->via_size_spin_button),
					FROM_PCB_UNITS(min_via_size), max);
	}

static void
via_size_cb(GtkWidget *widget, SizesDialog *sd)
	{
	gdouble	max, min, via_size;
	gint	max_hole_size;

	gtk_spin_button_get_range(GTK_SPIN_BUTTON(sd->via_hole_spin_button),
				&min, &max);
	via_size = gtk_spin_button_get_value(GTK_SPIN_BUTTON(widget));
	max_hole_size = TO_PCB_UNITS(via_size) - MIN_PINORVIACOPPER;
	if (max_hole_size < max)
		gtk_spin_button_set_range(GTK_SPIN_BUTTON(sd->via_hole_spin_button),
					min, FROM_PCB_UNITS(max_hole_size));
	}
...
use_temp_cb(GtkWidget *button, gpointer data)
  ...
  */
void
gui_route_style_dialog(gint index, RouteStyleType *temp_rst)
	{
	...
	vbox1 = gui_category_vbox(vbox, _("Sizes"), 4, 2, TRUE, TRUE);
	table = gtk_table_new(4, 2, FALSE);
	gtk_table_set_col_spacings(GTK_TABLE(table), 6);
	gtk_table_set_row_spacings(GTK_TABLE(table), 3);

	/* XXX Scale these based on units_mm?? */
	gui_table_spin_button(table, 0, 0,
	...
	if (!editing_temp)
		{
		vbox1 = gui_category_vbox(vbox, _("Temporary Styles"),
					4, 2, TRUE, TRUE);
		label = gtk_label_new("");
		gtk_label_set_use_markup(GTK_LABEL(label), TRUE);
		...
		gui_check_button_connected(vbox1, &sd->set_temp1_button, FALSE,
				TRUE, FALSE, FALSE, 0,
				use_temp_cb, GINT_TO_POINTER(1), _("Temp1"));
		gui_check_button_connected(vbox1, &sd->set_temp2_button, FALSE,
				TRUE, FALSE, FALSE, 0,
				use_temp_cb, GINT_TO_POINTER(2), _("Temp2"));

		vbox1 = gui_category_vbox(vbox, _("Default Style"),
					4, 2, TRUE, TRUE);
		label = gtk_label_new("");
		gtk_label_set_use_markup(GTK_LABEL(label), TRUE);
		s = g_strdup_printf(_("<small>"
		...
		g_free(s);
		gtk_box_pack_start(GTK_BOX(vbox1), label, FALSE, FALSE, 0);
		gui_check_button_connected(vbox1, &set_default_button, FALSE,
				TRUE, FALSE, FALSE, 0,
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_OK)
		...
		if (index < NUM_STYLES && !set_temp1 && !set_temp2)
			{
			string = gui_entry_get_text(sd->name_entry);
			SaveFree(rst->Name);
			rst->Name = StripWhiteSpaceAndDup (string);
			pcb_use_route_style(rst);
			SetChangedFlag(True);
			gui_route_style_set_button_label(rst->Name, index);
			}
		else
			{
			pcb_use_route_style(rst);
			gui_route_style_set_temp_style(rst, set_temp1 ? 0 :
