...
/***********************************************************************
*
* Footprint and map management
*
***********************************************************************/

/*************************************************************
* Loads database of footprints and the corresponding map files
**************************************************************/
static int
...
/* Parses single line of map file (propject map or component map) */
static int
...
/* Parses option in PCB  preferences file */
...
/* Prepares list of paths, where component library is located */
static char *
scad_libpaths ()
{
  char *libpaths;
  ...
  if (fc) {
    ...
    while (fgets (line, sizeof (line), fc)) {
      ...
      if (scad_parse_option (line, &opt, &gtkpaths) && !strcmp (opt, "stdlib"))
      {
        if (opt)
          free (opt);
        break;
  ...
  if (!gtkpaths) {
    return strdup (Settings.LibraryPath);
  }
  else {

    libpaths = (char*)calloc (1, strlen (Settings.LibraryPath) + strlen (gtkpaths) + strlen (PCB_PATH_DELIMETER) + 1);
...
/************************************************************
* Expands all variables in the model description
...
/************************************************************
* Component angle calculation
* - calculates angle between first pin or pad and center of component for footprint and element placed on the board
* - returns difference of these angles as rotation of the component
* for components with single pin or single square pad we are not able to detect the angle
************************************************************/
static float
...
/************************************************************
* Alternative component angle calculation
