...
/*!
 * \brief Scaling function - all output is in milimeters.
 */
...
/*!
 * \brief Export OpenSCAD filter options.
 */
 ...
 * - calculates positions of layers, depending on options and number of
 *   layer groups.
...
scad_do_export (HID_Attr_Val * options)
{
  ...
  group_data[GetLayerGroupNumberByNumber (max_copper_layer + TOP_SIDE)].solder = 1;
  group_data[GetLayerGroupNumberByNumber (max_copper_layer + BOTTOM_SIDE)].component = 1;
...
/*!
 * \brief Helper function - draws the line or collects the line segments.
 *
 * - on outline layer the line segments are collected and later used to
 *   draw board outline.
 * - otherwise it is drawn as rotated box with circle at the end(s),
 *   depending on mode:
 *   -- on single line segment or last polyline segment caps are drawn
 *      on both ends.
...
/*!
 * \brief Emit the cylinder - its appearance depends on layer it is
 * located on.
 *
...
/*!
 * \brief Helper function - creates extruded polygon.
 */
static void
