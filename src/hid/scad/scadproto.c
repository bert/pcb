...
scad_write_prologue ()
{
  ...
  fputs ("module line_segment_r(length, width, thickness, x, y, a, bd, c1, c2) {\n"
...
scad_generate_board ()
{
  ...
  fprintf (scad_output, "\ttranslate ([0, 0, %f]) linear_extrude(height=%f) board_outline();", -BOARD_THICKNESS / 2., BOARD_THICKNESS);
  fprintf (scad_output, "}\n\n");
}
...
scad_generate_mask ()
{
  ...
  fprintf (scad_output, "\ttranslate ([0, 0, %f]) linear_extrude(height=%f) board_outline();", -MASK_THICKNESS / 2., MASK_THICKNESS);
  fprintf (scad_output, "}\n\n");
}
