/*!
 * \file src/hid/gtk/gui-elements-window.c
 *
 * \brief Routines for an element window.
 *
 * <hr>
 *
 * <h1><b>Copyright.</b></h1>\n
...
/*!
 * \brief The columns of our list store, which stores a list of elements */
enum
{
...
/*!
 * \brief Callback function for selecting an element.
 *
 * \param element The element to select.
 */
static void
op_element_select (ElementType *element)
{
  SelectElement (element, true);
}

/*!
 * \brief Callback function for deselecting an element.
 *
 * \param element The element to deselect.
 */
static void
op_element_deselect (ElementType *element)
{
  SelectElement (element, false);
}

/*!
 * \brief Callback function for applying the given operation to the given
 ...
 */
static void
proc_selected (GtkTreeModel *model,
               GtkTreePath *path,
               GtkTreeIter *iter,
               gpointer _op)
{
  ...
  gtk_tree_model_get (model, iter, ELEMLIST_ELEMENT_P_COLUMN, &element, -1);

  op(element);
}

/*!
 * \brief Callback function for select button click event.
 */
static void
sel_button_clicked (GtkButton *button_select, gpointer _self)
{
...
/*!
 * \brief Callback function for deselect button click event.
 */
static void
desel_button_clicked (GtkButton *button_select, gpointer _self)
{
  GhidElementsWindow *self = GHID_ELEMENTS_WINDOW (_self);
  GtkTreeSelection *sel = gtk_tree_view_get_selection (GTK_TREE_VIEW(self->treeview));
...
/*!
 * \brief Filter function for the filtered tree model.
 *
 * Returns true if the item complies with the current search parameters.
 *
 ...
 */
static gboolean
elemlist_filter (GtkTreeModel *model,
                 GtkTreeIter *iter,
                 gpointer _self)
{
  char *name;
  ...
  gtk_tree_model_get (model, iter, ELEMLIST_NAME_COLUMN, &name, -1);
  ...
  g_object_get (self->search, "regex", &regex_mode, "case-sensitive",
                &case_sensitive, NULL);
...
/*!
 * \brief Compare function for sorting items in the treeview by name.
 *
 ...
 */
static gint
elemlist_sort_name_cmp (GtkTreeModel *model,
                        GtkTreeIter *a,
                        GtkTreeIter *b,
                        gpointer ud)
{
  ...
  gtk_tree_model_get (model, a, ELEMLIST_NAME_COLUMN, &a_name, -1);
  gtk_tree_model_get (model, b, ELEMLIST_NAME_COLUMN, &b_name, -1);
...
/*!
 * \brief Find an element in the element store.
 *
 * \param store The store to search.
 * \param needle The element to find.
 * \param _iter Pointer to the iter to populate with the located item.
 * Can be NULL if not required.
 *
 * \return Whether the element was found.
 */
static bool
elemlist_find_element (GtkTreeModel *store, 
  ...
  while (valid)
    {
      ...
      gtk_tree_model_get (store, iter, ELEMLIST_ELEMENT_P_COLUMN, &element, -1);
...
/*!
 * \brief Synchronise the list store with the main list of elements.
 *
 * \param self The GhidElementsWindow instance.
 */
static void
update_list (GhidElementsWindow *self)
{
  gboolean valid;
  GtkTreeIter iter;
      ...
      if (elemlist_find_element (GTK_TREE_MODEL (self->store), element, &iter))
        {
          ...
          gtk_tree_model_get (GTK_TREE_MODEL (self->store), &iter, ELEMLIST_NAME_COLUMN, &old_name, -1);


          if (g_strcmp0(name, old_name) != 0)
            {
              /* Name has changed */
              gtk_list_store_set (self->store, &iter, ELEMLIST_NAME_COLUMN, name, -1);
            }
  ...
  while (valid)
    {
      ...
      gtk_tree_model_get (GTK_TREE_MODEL(self->store), &iter, ELEMLIST_ELEMENT_P_COLUMN, &store_element, -1);
...
/*!
 * \brief Create the treeview widget.
 *
 * \param self The GhidElementsWindow instance.
 */
static void
create_treeview (GhidElementsWindow *self)
{
  ...
  viewcol = gtk_tree_view_column_new_with_attributes (_("Element Name"),
                                                      renderer,
...
/*!
 * \brief Callback for when the search parameters change.
 *
 * \param _self The GhidElementsWindow instance.
 */
static void
update_search (gpointer *_self)
{
  GhidElementsWindow *self = GHID_ELEMENTS_WINDOW (_self);
  gboolean regex_mode, case_sensitive;
...
/*!
 * \brief Show the elements window, creating it if necessary.
 */
void
ghid_elements_window_show (void)
{
  if (elemwin == NULL)
    {
...
/*!
 * \brief GhidElementsWindowClass initialiser.
 */
static void
ghid_elements_window_class_init (GhidElementsWindowClass *klass)
{
}

/*!
 * \brief Window's "configure-event" signal handler.
 */
static gboolean
...
/*!
 * \brief GhidElementsWindow initialiser.
 */
static void
ghid_elements_window_init (GhidElementsWindow *self)
{
  ...
  gtk_window_resize (GTK_WINDOW(self), ghidgui->elements_window_width,
...
/*!
 * \brief HID action function for when elements are modified.
 */
static int
elements_changed_cb (int argc, char** argv, Coord x, Coord y)
{
