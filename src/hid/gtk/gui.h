...
#define	DRAW_X(x)         (int)((SIDE_X(x) - gport->view.x0) / gport->view.coord_per_px)
#define	DRAW_Y(y)         (int)((SIDE_Y(y) - gport->view.y0) / gport->view.coord_per_px)

#define	EVENT_TO_PCB_X(x) SIDE_X((int)((x) * gport->view.coord_per_px + gport->view.x0))
#define	EVENT_TO_PCB_Y(y) SIDE_Y((int)((y) * gport->view.coord_per_px + gport->view.y0))
...
typedef struct
{
  ...
  char *name_label_string;

  bool adjustment_changed_holdoff,
...
typedef struct
{
  ...
  unsigned tooltip_update_timeout_id;
}
GHidPort;
...
void ghid_window_set_name_label (char * name);
void ghid_interface_set_sensitive (bool sensitive);
...
void ghid_config_layer_name_update (char * name, int layer);
void ghid_config_groups_changed(void);

void ghid_config_init (void);
void ghid_config_files_write (void);
void ghid_config_files_read (int * argc, char *** argv);
...
*/
void ghid_status_line_set_text (const char * text);
void ghid_cursor_position_label_set_text (char * text);
void ghid_cursor_position_relative_label_set_text (char * text);
...
void ghid_create_abort_dialog (char *);
bool ghid_check_abort (void);
...
bool ghid_note_event_location    (GdkEventButton *ev);
bool ghid_port_key_press_cb      (GtkWidget *drawing_area, GdkEventKey *kev, void *data);
bool ghid_port_key_release_cb    (GtkWidget *drawing_area, GdkEventKey *kev, void *data);
bool ghid_port_button_press_cb   (GtkWidget *drawing_area, GdkEventButton *ev, void *data);
bool ghid_port_button_release_cb (GtkWidget *drawing_area, GdkEventButton *ev, void *data);

bool ghid_port_window_enter_cb (GtkWidget *widget, GdkEventCrossing *ev, GHidPort *out);
bool ghid_port_window_leave_cb (GtkWidget *widget, GdkEventCrossing *ev, GHidPort *out);
bool ghid_port_window_motion_cb (GtkWidget *widget, GdkEventMotion *ev, GHidPort *out);
bool ghid_port_window_mouse_scroll_cb (GtkWidget *widget, GdkEventScroll *ev, GHidPort *out);

bool ghid_port_drawing_area_configure_event_cb (GtkWidget *widget, GdkEventConfigure *ev, GHidPort *out);
...
void ghid_dialog_message (char *message);
bool ghid_dialog_confirm (char *message, char *cancelmsg, char *okmsg);
...
int   ghid_dialog_confirm_all (char *message);
char *ghid_dialog_input (const char *prompt, const char *initial);
...
/* gui-drc-window.c */
void ghid_drc_window_show (bool raise);
...
/* gui-utils.c
*/
bool dup_string          (char **dst, const char *src);
...
bool ghid_is_modifier_key_sym (int ksym);
...
char *ghid_get_color_name         (GdkColor *color);
void  ghid_map_color_string       (char *color_string, GdkColor *color);
char *ghid_entry_get_text         (GtkWidget *entry);
void  ghid_check_button_connected (GtkWidget *box, GtkWidget **button,
                                   bool active, bool pack_start,
                                   bool expand, bool fill, int pad,
                                   void (*cb_func) (GtkToggleButton *, void *), void *data,
                                   char *string);
void ghid_button_connected (GtkWidget *box, GtkWidget ** button,
			    bool pack_start, bool expand,
			    bool fill, int pad, void (*cb_func) (void *),
...
void ghid_spin_button (GtkWidget *box, GtkWidget ** spin_button,
		       float value, float low, float high, float step0,
		       float step1, int digits, int width,
		       void (*cb_func) (GtkSpinButton *, void *), void *data,
		       bool right_align, char *string);
void ghid_table_coord_entry (GtkWidget *table, int row, int column,
		 	    GtkWidget ** coord_entry, Coord value,
			    Coord low, Coord high, enum ce_step_size, int width,
			    void (*cb_func) (GHidCoordEntry *, void *), void *data,
			    bool right_align, char *string);
void ghid_table_spin_button (GtkWidget *box, int row, int column,
			     GtkWidget ** spin_button, float value,
			     float low, float high, float step0,
			     float step1, int digits, int width,
			     void (*cb_func) (GtkSpinButton *, void *), void *data,
			     bool right_align, char *string);
...
void ghid_range_control (GtkWidget *box, GtkWidget ** scale_res,
			 ...
			 bool set_draw_value, int digits,
			 bool pack_start, bool expand, bool fill,
			 unsigned int pad, float value, float low, float high,
			 float step0, float step1, void (*cb_func) (),
			 void *data);
GtkWidget *ghid_scrolled_vbox               (GtkWidget *box, GtkWidget ** scr, GtkPolicyType h_policy, GtkPolicyType v_policy);
GtkWidget *ghid_framed_vbox                 (GtkWidget *box, char *label,
                                             int frame_border_width, bool frame_expand,
                                             int vbox_pad, int vbox_border_width);
GtkWidget *ghid_framed_vbox_end             (GtkWidget *box, char *label,
                                             int frame_border_width,
                                             bool frame_expand, int vbox_pad,
                                             int vbox_border_width);
GtkWidget *ghid_category_vbox               (GtkWidget *box, const char *category_header,
                                             int header_pad, int box_pad,
                                             bool pack_start, bool bottom_pad);
GtkWidget *ghid_notebook_page               (GtkWidget *tabs, const char *name, int pad, int border);
GtkWidget *ghid_framed_notebook_page        (GtkWidget *tabs, char *name,
                                             int border, int frame_border,
                                             int vbox_pad, int vbox_border);
GtkWidget *ghid_scrolled_text_view          (GtkWidget *box, GtkWidget ** scr,
                                             GtkPolicyType h_policy,
                                             GtkPolicyType v_policy);
void       ghid_text_view_append            (GtkWidget *view, char *string);
void       ghid_text_view_append_strings    (GtkWidget *view, char ** string,
				    int n_strings);
...
void ghid_dialog_report                      (char *title, char *message);
void ghid_label_set_markup                   (GtkWidget *label, const char *text);
...
void ghid_netlist_window_show                (GHidPort *out, bool raise);
void ghid_netlist_window_update              (bool init_nodes);

LibraryMenuType *ghid_get_net_from_node_name (char *name, bool);
void ghid_netlist_highlight_node             (char *name);


/* gui-command-window.c */
void ghid_handle_user_command (bool raise);
void ghid_command_window_show (bool raise);
char *ghid_command_entry_get (char *prompt, char *command);
void ghid_command_use_command_window_sync (void);

/* gui-keyref-window.c */
void ghid_keyref_window_show (bool raise);

/* gui-library-window.c */
void ghid_library_window_create (GHidPort *out);
void ghid_library_window_show (GHidPort *out, bool raise);


/* gui-log-window.c */
void ghid_log_window_create ();
void ghid_log_window_show (bool raise);
...
bool ghid_drawing_area_expose_cb  (GtkWidget *, GdkEventExpose *, GHidPort *);
void ghid_port_drawing_realize_cb (GtkWidget *, void *);
...
void ghid_port_rotate             (void *ball, float *quarternion, void *userdata);
void ghid_view_2d                 (void *ball, bool view_2d, void *userdata);
