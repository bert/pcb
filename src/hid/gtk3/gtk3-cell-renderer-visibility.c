/*!
 * \file src/hid/gtk3/gtk3-cell-renderer-visibility.c
 *
...
ghid_cell_renderer_visibility_get_size (GtkCellRenderer *cell,
                                        ...
                                        gint            *height)
{
  /*! \todo GtkStyle is deprecated in GTK3, use GtkStyleContext.
   * gtk_widget_get_style is deprecated in GTK3,
   * use gtk_widget_get_style_context.
   * Usually the GtkStyle for a widget is the same as the default style
   * that is set by GTK+ and modified the theme engine.
...
*/
  ...
  w = VISIBILITY_TOGGLE_SIZE + 2 * (xpad);
  h = VISIBILITY_TOGGLE_SIZE + 2 * (ypad);
                                      ...
                                      GtkCellRendererState  flags)
{
  ...
  ghid_cell_renderer_visibility_get_size (cell,
                                          widget,
                                          cell_area,
                                          &toggle_rect.x,
                                          &toggle_rect.y,
  ...
  if (gdk_rectangle_intersect (expose_area, cell_area, &draw_rect))
  {
    GdkRGBA color;
    /*!
     * \todo \c gdk_cairo_create is deprecated:
     * Use \c gdk_window_begin_draw_frame() and
     * \c gdk_drawing_context_get_cairo_context() instead.
     */
//    cairo_t *cr = gdk_cairo_create (window);
    ...
    /*!
     * \todo \c gdk_color_parse is deprecated:
     * Use \c gdk_rgba_parse instead.
     */
    gdk_rgba_parse (pcb_cell->color, &color);
