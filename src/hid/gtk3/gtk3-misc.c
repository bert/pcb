/*!
 * \file src/hid/gtk3/gtk3-misc.c
 *
 * \brief Miscellaneous stuff for the GTK3 UI.
 *
...
gport_set_cursor (GdkCursorType shape)
{
  ...
  if (shape > GDK_LAST_CURSOR)
  {
    if (shape == CUSTOM_CURSOR_CLOCKWISE)
      gport->X_cursor = gdk_cursor_new_from_pixmap (XC_clock_source,
                                                    XC_clock_mask,
                                                    &fg,
                                                    &bg,
                                                    ICON_X_HOT,
      ...
      gport->X_cursor = gdk_cursor_new_from_pixmap (XC_hand_source,
                                                    XC_hand_mask,
                                                    &fg,
                                                    &bg,
                                                    ICON_X_HOT,
      ...
      gport->X_cursor = gdk_cursor_new_from_pixmap (XC_lock_source,
                                                    XC_lock_mask,
                                                    &fg,
                                                    &bg,
                                                    ICON_X_HOT,
...
/*!
 * \brief If user hits a key instead of the mouse button, we'll abort
 * unless it's the enter key (which accepts the current crosshair
 * location).
 */
static gboolean
loop_key_press_cb (GtkWidget *drawing_area, GdkEventKey *kev, GMainLoop **loop)
{
...
/*!
 * \brief User hit a mouse button in the Output drawing area, so quit
 * the loop and the cursor values when the button was pressed will be
 * used.
 */
static gboolean
loop_button_press_cb (GtkWidget *drawing_area, GdkEventButton *ev, GMainLoop **loop)
{
  if (g_main_loop_is_running (*loop))
    g_main_loop_quit (*loop);
...
/*!
 * \brief Run a glib GMainLoop which intercepts key and mouse button
 * events from the top level loop.
 *
 * When a mouse or key is hit in the Output drawing area, quit the loop
 * so the top level loop can continue and use the the mouse pointer
 * coordinates at the time of the mouse button event.
 */
static gboolean
run_get_location_loop (const gchar *message)
{
   ...
   * and mouse so we can install our own handlers here.
   * Also set the control interface insensitive so all the user can do
   * is hit a key or mouse button in the Output drawing area.
   */
  ...
  button_handler = g_signal_connect (G_OBJECT (gport->drawing_area),
                                     "button_press_event",
                                     G_CALLBACK (loop_button_press_cb),
                                     &loop);

  key_handler = g_signal_connect (G_OBJECT (gport->top_window),
                                  "key_press_event",
                                  G_CALLBACK (loop_key_press_cb),
                                  &loop);
 ...
 * \brief .
 *
 * \todo The abort dialog isn't implemented yet in the Gtk port.
 */
void
...
/*!
 * \brief /* Abort isn't implemented, so never abort.
 */
gboolean
...
/*!
 * \brief Output of cursor position.
 */
void
