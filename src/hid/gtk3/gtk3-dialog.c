/*!
 * \file src/hid/gtk3/gtk3-dialog.c
 *
 * \brief Provides a dialog window for the GTK3 UI.
 *
...
ghid_dialog_input (const char * prompt, const char * initial)
{
  ...
  gtk_label_set_markup (GTK_LABEL (label), prompt ? prompt : _("Enter something"));
 ...
 * \brief .
 *
 * \warning Caller must g_free() the returned filename.
 */
gchar *
 ...
 * \brief .
 *
 * \warning Caller must g_slist_free() the returned list.
 */
GSList *
 ...
 */
gchar *
ghid_dialog_file_select_save (gchar * title, gchar ** path, gchar * file, gchar * shortcuts)
{
  GtkWidget *dialog;
  gchar *result = NULL, *folder, *seed;
  GHidPort *out = &ghid_port;
...
/*!
 * \brief How many files and directories to keep for the shortcuts.
 */
#define NHIST 8
 ...
 */
gchar *
ghid_fileselect (const char *title, const char *descr, char *default_file, char *default_ext, const char *history_tag, int flags)
{
  GtkWidget *dialog;
  gchar *result = NULL;
  GHidPort *out = &ghid_port;
