/*!
 * \file src/hid/gtk3/gtk3-log-window.c
 *
 * \brief Provides a log window for the GTK3 UI.
 *
 ...
 */
static gint
log_window_configure_event_cb (GtkWidget *widget, GdkEventConfigure *ev, gpointer data)
{
...
ghid_log_window_create ()
{
  ...
  g_signal_connect (G_OBJECT (log_window),
                    "destroy",
                    G_CALLBACK (log_destroy_cb),
                    NULL);

  g_signal_connect (G_OBJECT (log_window),
                    "configure_event",
                    G_CALLBACK (log_window_configure_event_cb),
                    NULL);
  ...
  log_text = ghid_scrolled_text_view (vbox,
                                      NULL,
