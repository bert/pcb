/*!
 * \file src/hid/gtk3/gtk3-netlist-window.c
 *
 * \brief Provides a netlist window for the GTK3 UI.
 *
 ...
 * will be displayed in a right treeview.
 * If a "Select on layout" button is pressed, the net that is selected
 * in the left treeview will be drawn selected on the layout.
 *
 * Gtk separates the data model from the view in its treeview widgets so
 * here we maintain two data models.
 * The net data model has pointers to all the nets in the layout and the
 * node data model keeps pointers to all the nodes for the currently
 * selected net.
 * By updating the data models the net and node gtk treeviews handle
 * displaying the results.
 *
 * The netlist window code has a public interface providing hooks so
 * PCB code can control the net and node treeviews:
 *
 ...
 *   data model and search each net for the given node_name.
 *   If found and enabled_only is true, make the net treeview scroll
 *   to and highlight (select) the found net.
 *   Return the found net.
 *
 * - ghid_netlist_highlight_node().
 *   Given some PCB internal pointers (not really a good gui api here)
 *   look up a node name determined by the pointers and highlight the
 *   node in the node treeview.
 *   By using ghid_get_net_from_node_name() to look up the node, the net
 *   the node belongs to will also be highlighted in the net treeview.
 *
...
/*!
 * \brief The netlist nodes (LibraryEntryType) data model.
 *
 ...
 * to the net.
 * Loading the new model will update the right treeview with all the new
 * node names (C100-1, R100-1, etc).
 *
 ...
 * data, and possibly other things also.
 * The mapping is that the layout netlist data structure is a
 * LibraryType which contains an allocated array of LibraryMenuType
 * structs.
 * Each of these structs represents a net in the netlist and contains an
 * array of LibraryEntryType structs which represent the nodes
 ...
 * Where for example Menu[] names would be nets GND, Vcc, etc and
 * Entry[] names would be nodes C101-1, R101-2, etc.
 */
...
/*!
 * \brief Given a net in the netlist (a LibraryMenuType) put all the
 * Entry[] names (the nodes) into a newly created node tree model.
 */
static GtkTreeModel *
...
/*!
 * \brief When there's a new node to display in the node treeview, call
 * this.
 *
...
node_model_update (LibraryMenuType *menu)
{
  ...
  /* We could be using gtk_list_store_clear() on the same model, but
   * it's just as easy that we've created a new one and here unref the
   * old one. */
  if (model)
...
/*!
 * \brief Callback when the user clicks on a PCB node in the right node
 * treeview.
 */
static void
node_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
   ...
   * Look up node_name to make sure it still exists.
   * This toggling can get out of sync if a node is toggled selected,
   * then the net that includes the node is selected then unselected. */
...
/*!
 * \todo The enable and disable all nets.
 * Can't seem to get how that's supposed to work, but it'll take
 * updating the NET_ENABLED_COLUMN in the net_model.
 * Probably it should be made into a gpointer and make a text renderer
 * for it and just write a '*' or a ' ' similar to the the Xt PCB
 * scheme.
 * Or better, since it's an "all nets" function, just have a
 * "Disable all nets" toggle button and don't mess with the
...
net_model_create (void)
{
  ...
  prefix_hash = g_hash_table_new_full (g_str_hash,
                                       g_str_equal,
                                       g_free,
...
/*!
 * \brief Called when the user double clicks on a net in the left treeview.
 */
static void
net_selection_double_click_cb (GtkTreeView *treeview, GtkTreePath *path, GtkTreeViewColumn *col, gpointer data)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
  gchar *str;
...
/*!
 * \brief Called when the user clicks on a net in the left treeview.
 */
static void
net_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
 ...
 /* PCB is highlighting, user is not selecting. */
   ...
   * netlist row, and create a new node model from the net entries and
   * insert that model into the node view.
   * Delete old entry model. */
...
netlist_disable_all_cb (GtkToggleButton *button, gpointer data)
{
  ...
  if (gtk_tree_model_get_iter_first (net_model, &iter))
    do
    {
      gtk_tree_store_set (GTK_TREE_STORE (net_model),
                          &iter,
                          NET_ENABLED_COLUMN,
                          active ? "*" : "",
                          -1);
...
/*!
 * \brief Select on the layout the current net treeview selection.
 */
static void
...
static gboolean
node_get_node_from_name_helper (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
  LibraryMenuType *net;
  LibraryEntryType *node;
...
node_get_node_from_name (gchar *node_name, LibraryMenuType **node_net)
{
  ...
  /* Have to force the netlist window created because we need the
   * treeview models constructed to do the search. */
  ghid_netlist_window_create (gport);

  /* Now walk through node entries of each net in the net model looking
   * for the node_name. */
  state.found = 0;
...
static gint
netlist_window_configure_event_cb (GtkWidget *widget, GdkEventConfigure *ev, gpointer data)
{
...
netlist_close_cb (GtkWidget *widget, gpointer data)
{
  ...
  /* For now, we are the only consumer of this API, so we can just do
   * this. */
  ghid_cancel_lead_user ();
}
...
ghid_netlist_window_create (GHidPort *out)
{
  ...
  g_signal_connect (G_OBJECT (netlist_window),
                    "destroy",
                    G_CALLBACK (netlist_destroy_cb),
                    out);
  ...
  g_signal_connect (G_OBJECT (netlist_window),
                    "configure_event",
                    G_CALLBACK (netlist_window_configure_event_cb),
                    NULL);
  ...
  /*! \todo Don't expand all, but record expanded states when window is
   *  destroyed and restore state here. */
  gtk_tree_view_expand_all (treeview);

  ghid_scrolled_selection (treeview,
                           hbox,
                           GTK_SELECTION_SINGLE,
                           GTK_POLICY_NEVER,
                           GTK_POLICY_AUTOMATIC,
                           net_selection_changed_cb,
                           NULL);
  ...
  g_signal_connect (G_OBJECT (button),
                    "clicked",
                    G_CALLBACK (netlist_rip_up_cb),
                    GINT_TO_POINTER (0));
  
  ghid_check_button_connected (vbox,
                               &disable_all_button,
                               FALSE,
                               TRUE,
                               FALSE,
...
static gboolean
hunt_named_node (GtkTreeModel *model, GtkTreePath *path, GtkTreeIter *iter, gpointer data)
{
...
ghid_get_net_from_node_name (gchar *node_name, gboolean enabled_only)
{
  ...
  /* Have to force the netlist window created because we need the
   * treeview models constructed so we can find the LibraryMenuType
   * pointer the caller wants. */
  ghid_netlist_window_create (gport);
   ...
   * expecting normal select action to happen?
   * Or should the node treeview also get updated?
   * Original PCB code just tries to highlight. */
...
/*!
 * \brief PCB LookupConnection code in find.c calls this if it wants a
 * node and its net highlighted.
 */
void
ghid_netlist_highlight_node (gchar *node_name)
{
  ...
  /* We've found the net containing the node, so update the node
   * treeview to contain the nodes from the net.
   * Then we have to find the node in the new node model so we can
   * highlight it. */
  node_model_update (net);
  ...
  if (gtk_tree_model_get_iter_first (node_model, &iter))
    do
    {
      ...
      { /* Found it, so highlight it. */
          ...
          gtk_tree_view_scroll_to_cell (node_treeview,
                                        path,
                                        NULL,
                                        TRUE,
                                        0.5,
...
/*!
 * \brief If code in PCB should change the netlist, call this to update
...
ghid_netlist_window_update (gboolean init_nodes)
{
  ...
  /*! \todo Check if the select callback does this for us. */
  if (init_nodes)
...
{
  /* We get called before the GUI is up when exporting from the
   * command-line. */
