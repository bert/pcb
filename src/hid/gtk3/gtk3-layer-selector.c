/*!
 * \file src/hid/gtk3/gtk3-layer-selector.c
 *
 ...
 *
 * \todo We should not have to do this within a simple widget, and this
 * quick hack workaround breaks the widget's abstraction from the rest
 * of the application :(
 */
static void
 ...
 *
 *  \param [in] ls            The selector to be acted on.
 *  \param [in] user_id       An ID used to identify the layer; will be
 *                            passed to selection/visibility callbacks.
 *  \param [in] name          The name of the layer; will be used on
 *                            selector and menus.
 *  \param [in] color_string  The color of the layer on selector.
 *  \param [in] visibile      Whether the layer is visible.
 *  \param [in] selectable    Whether the layer appears in menus and can
 *                            be selected.
...
/*!
 * \brief Helper function for toggling visibility of layers.
 *
 * Used internally.
 */
static gboolean
...
ghid_layer_selector_toggle_layer (GHidLayerSelector *ls, gint user_id)
{
  ...
  gtk_tree_model_foreach (GTK_TREE_MODEL (ls->list_store), toggle_foreach_func, &data);
}


/*!
 * \brief Helper function for selecting layers.
 *
 * Used internally.
 */
static gboolean
...
ghid_layer_selector_select_layer (GHidLayerSelector *ls, gint user_id)
{
  ...
  gtk_tree_model_foreach (GTK_TREE_MODEL (ls->list_store), select_foreach_func, &data);
}
