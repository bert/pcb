/*!
 * \file src/hid/gtk3/gtk3-library-window.c
 *
...
static gint
library_window_configure_event_cb (GtkWidget *widget, GdkEventConfigure *ev, gpointer data)
{
 ...
 */
static void
library_window_callback_response (GtkDialog *dialog, gint arg1, gpointer user_data)
{
  switch (arg1)
  {
 ...
 */
static gboolean
lib_model_filter_visible_func (GtkTreeModel *model, GtkTreeIter *iter, gpointer data)
{
  GhidLibraryWindow *library_window = (GhidLibraryWindow *) data;
  const gchar *compname;
  gchar *compname_upper;
  gchar *text_upper;
  gchar *pattern;
  const gchar *text;
  gboolean ret;
 ...
 */
static void
tree_row_activated (GtkTreeView *tree_view, GtkTreePath *path, GtkTreeViewColumn *column, gpointer user_data)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
 ...
 */
static gboolean
tree_row_key_pressed (GtkTreeView *tree_view, GdkEventKey *event, gpointer user_data)
{
 ...
 */
static void
library_window_callback_tree_selection_changed (GtkTreeSelection *selection, gpointer user_data)
{
  GtkTreeModel *model;
  GtkTreeIter iter;
   ...
   * LoadElementToBuffer(). */
  m4_args = g_strdup_printf ("'%s' '%s' '%s'", EMPTY (entry->Template), EMPTY (entry->Value), EMPTY (entry->Package));
 ...
 */
static void
library_window_callback_filter_entry_changed (GtkEditable *editable, gpointer user_data)
{
 ...
 */
static void
library_window_callback_filter_button_clicked (GtkButton *button, gpointer user_data)
{
  GhidLibraryWindow *library_window = GHID_LIBRARY_WINDOW (user_data);
...
create_lib_tree_model (GhidLibraryWindow *library_window)
{
  ...
  tree = gtk_tree_store_new (N_MENU_COLUMNS, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_POINTER, G_TYPE_POINTER);
    ...
    do
    {
      ...
      *tok_end = '\0';
      ...
      if (gtk_tree_model_iter_children (GTK_TREE_MODEL (tree), &e_iter, iter))
        do
        {
          gtk_tree_model_get (GTK_TREE_MODEL (tree), &e_iter, MENU_TOPPATH_COLUMN, &name, -1);

          if (strcmp (name, menu->directory) != 0)
            continue;

          gtk_tree_model_get (GTK_TREE_MODEL (tree), &e_iter, MENU_SUBPATH_COLUMN, &name, -1);
      ...
      else
      {
        ...
        gtk_tree_store_set (tree,
                            ...
                            rel_path,
                            MENU_NAME_COLUMN,
                            tok_end == rel_path ? g_path_get_basename(menu->directory) : tok_start,
                            MENU_LIBRARY_COLUMN,
 ...
 */
static void
library_window_callback_refresh_library (GtkButton *button, gpointer user_data)
{
  GhidLibraryWindow *library_window = GHID_LIBRARY_WINDOW (user_data);
  GtkTreeModel *model;

  /* Rescan the libraries for symbols. */
  /*! \todo How do we do this in PCB ?  */
...
create_lib_treeview (GhidLibraryWindow *library_window)
{
  ...
  g_signal_connect (selection,
                    "changed",
                    G_CALLBACK (library_window_callback_tree_selection_changed),
                    library_window);
  ...
  /* Init the event source for footprint filter. */
  library_window->filter_timeout = 0;
  ...
  g_signal_connect (button,
                    "clicked",
                    G_CALLBACK (library_window_callback_filter_button_clicked),
                    library_window);
...
static GObject *
library_window_constructor (GType type, guint n_construct_properties, GObjectConstructParam *construct_params)
{
  GObject *object;
