/*!
 * \file src/hid/gtk3/gtk3-route-style-selector.c
 *
 * \brief Implementation of the route style selector widget.
 *
...
init_radio_groups (GHidRouteStyleSelector *rss)
{
  ...
  gtk_radio_button_set_group (GTK_RADIO_BUTTON (rss->null_button), rss->button_radio_group);
  rss->button_radio_group = gtk_radio_button_get_group (GTK_RADIO_BUTTON (rss->null_button));
}
...
dialog_style_changed_cb (GtkComboBox *combo, struct _dialog *dialog)
{
  ...
  ghid_coord_entry_set_value (GHID_COORD_ENTRY (dialog->mask_aperture_entry), style->rst->ViaMask);
}
...
ghid_route_style_selector_edit_dialog (GHidRouteStyleSelector *rss)
{
  ...
  if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_OK)
  {
    ...
    rst->ViaMask = ghid_coord_entry_get_value (GHID_COORD_ENTRY (dialog_data.mask_aperture_entry));
...
ghid_route_style_selector_new ()
{
  ...
  rss->null_action = gtk_radio_action_new ("RouteStyleXX", "", NULL, NULL, -1);
  rss->null_button = gtk_radio_button_new (rss->button_radio_group);
  init_radio_groups (rss);
  gtk_activatable_set_related_action (GTK_ACTIVATABLE (rss->null_button), GTK_ACTION (rss->null_action));
 ...
 */
void
ghid_route_style_selector_add_route_style (GHidRouteStyleSelector *rss, RouteStyleType *data)
{
 ...
 */
gboolean
ghid_route_style_selector_select_style (GHidRouteStyleSelector *rss, RouteStyleType *rst)
{
  GtkTreeIter iter;
