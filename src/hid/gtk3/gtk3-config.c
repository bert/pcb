/*!
 * \file src/hid/gtk3/gui-dialog-print.c
 *
 * \brief Provide a config dialog for the GTK3 GUI.
 *
...
static gchar *config_dir, *color_dir;
  ...
  /* CONFIG_Unused types are expected to be found in main_attribute_list
   * and will be assigned the type found there.
   * NULL value pointers here are also expected to be assigned values
   * from the main_attribute_list.
   */
...
/*! \todo Construct layer-names- in a list. */
...
config_file_open (gchar * mode)
{
  ...
  if (!config_dir)
  {
    ...
    if (!g_file_test (config_dir, G_FILE_TEST_IS_DIR) && MKDIR (config_dir, 0755) < 0)
    {
...
set_config_attribute (gchar * option, gchar * arg)
{
   ...
   * point convert.
   * Ultimately, data files will be POSIX and gui presentation (hence
   * the config file reals) will be in the users locale.
   */
  lc = localeconv ();
...
/*!
 * \brief Parse command line code borrowed from hid/common/hidinit.c.
 */
static void
...
config_sizes_apply (void)
{
  ...
  active = gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (use_increments_default_button));

  if (active)
  {
    save_increments (get_increments_struct (METRIC), get_increments_struct (IMPERIAL));
    ghidgui->config_modified = TRUE;
  }
...
static LayerGroupType *lg_monitor; /* Keep track if our working copy. */
/*! \todo Needs to be changed (new layout). */
...
/*!
 * \brief Construct a layer group string.
 *
 * Follow logic in WritePCBDataHeader(), but use g_string functions.
 */
static gchar *
...
config_layers_apply (void)
{
  ...
  /* Get each layer name entry and dup if modified into the PCB layer
   * names and, if to use as default, the Settings layer names.
   */
...
/*! \todo FIXME. */
  ...
  if (groups_modified) /* If any group radio buttons were toggled. */
  {
     ...
     */
    if (layer_groups.Number[bottom_group] <= 1 || layer_groups.Number[top_group] <= 1)
    {
    ...
    {
      Message (_("The 'top side' and 'bottom side' layers are not allowed\n"
...
ghid_config_groups_changed (void)
{
  ...
  /* Create a row of radio toggle buttons for layer.
   * So each layer can have an active radio button set for the group it
   * needs to be in.
   */
  for (layer = 0; layer < max_copper_layer; ++layer)
  {
    ...
    g_signal_connect(G_OBJECT (layer_entry[layer]), "activate",
                     G_CALLBACK (layer_name_entry_cb),
...
config_layers_tab_create (GtkWidget * tab_vbox)
{
  ...
  vbox1 = ghid_category_vbox (vbox, _("Operations on currently selected layer:"),
                              4, 2, TRUE, TRUE);
  ...
  vbox1 = ghid_category_vbox (vbox, _("Add new layer above currently selected layer:"),
                              4, 2, TRUE, TRUE);
...
config_color_load_cb (gpointer data)
{
  ...
  /* Recreate the colors config page to pick up new colors. */
  gtk_widget_destroy (config_colors_vbox);
  config_colors_tab_create (config_colors_tab_vbox);
...
{
  gchar *text = pcb_g_strdup_printf ("<b>%s</b>", Settings.grid_unit->in_suffix);
