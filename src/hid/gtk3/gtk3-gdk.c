...
ghid_draw_grid (BoxType * region)
{
  ...
  /* Test if grid is hidden. */
  if (!Settings.DrawGrid)
    return;

  /* test if zoomed out too far, points to close together. */
  if (Vz (PCB->Grid) < MIN_GRID_DISTANCE)
    return;

  /* Create a graphics context if we don't have one. */
  if (!priv->grid_gc)
  {
...
ghid_draw_bg_image (void)
{
  ...
  gint w_src;
  gint h_src;
  static gint w_scaled;
  static gint h_scaled;
  render_priv *priv = gport->render_priv;
  ...
  if (pixbuf)
    gdk_pixbuf_render_to_drawable (pixbuf, gport->drawable,
                                   priv->bg_gc,
                                   x, y, 0, 0,
                                   w - x, h - y,
...
ghid_use_mask (enum mask_mode mode)
{
  ...
  switch (mode)
  {
    ...
    case HID_MASK_CLEAR:
      ...
      /*! \todo Deprecated in GTK3. */
      gdk_draw_rectangle (gport->drawable, priv->mask_gc, TRUE,
                          0, 0,
...
void
ghid_draw_arc (hidGC gc, Coord cx, Coord cy, Coord xradius, Coord yradius, Angle start_angle, Angle delta_angle)
{
  gint vrx;
  gint vry;
  gint w;
...
ghid_draw_rect (hidGC gc, Coord x1, Coord y1, Coord x2, Coord y2)
{
  ...
  /*! \todo Deprecated in GTK3. */
  gdk_draw_rectangle (gport->drawable,
                      priv->u_gc,
                      FALSE,
                      x1,
                      y1,
...
ghid_fill_polygon (hidGC gc, int n_coords, Coord *x, Coord *y)
{
  ...
  /*! \todo Deprecated in GTK3. */
  gdk_draw_polygon (gport->drawable,
                    priv->u_gc,
                    1,
                    points,
                    n_coords);
...
redraw_region (GdkRectangle *rect)
{
  ...
  if (eleft > 0)
    /* Draw dead area on the left side. */
    /*! \todo Deprecated in GTK3. */
    gdk_draw_rectangle (gport->drawable,
                        priv->offlimits_gc,
                        1,
  ...
  if (eright < gport->width)
    /* Draw dead area on the right side. */
    /*! \todo Deprecated in GTK3. */
    gdk_draw_rectangle (gport->drawable,
                        priv->offlimits_gc,
                        1,
...
ghid_pinout_preview_expose (GtkWidget *widget, GdkEventExpose *ev)
{
  ...
  GdkDrawable *save_drawable; /*! \todo Deprecated in GTK3. */
...
GdkPixmap * /*! \todo Deprecated in GTK3. */
ghid_render_pixmap (int cx, int cy, double zoom, int width, int height, int depth)
{
  GdkPixmap *pixmap; /*! \todo Deprecated in GTK3. */
  GdkDrawable *save_drawable; /*! \todo Deprecated in GTK3. */
  view_data save_view;
  int save_width;
  int save_height;
