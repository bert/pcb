/*!
 * \file src/hid/gtk3/gtk3-main-menu.c
 *
 * \brief Implementation of main menu widget.
 *
  ...
  for (i = 0; i < res->c; ++i)
  {
    ...
    switch (resource_type (res->v[i]))
    {
      ...
      case   1:   /* no name, subres. */
         ...
         * a={"Ctrl-Q" "Ctrl<Key>q"}.
         * The first Gtk ignores.
         * The second needs to be translated. */
        if (tmp_res)
        ...
        for (j = 0; j < sub_res->c; j++)
        {
          ...
          if (resource_type (sub_res->v[j]) == 110)
          {
            ...
            if (action && strcmp (res_name, "checked") == 0)
              ...
              /* If we got this far it is supposed to be an X resource.
               * For now ignore it and warn the user. */
        ...
        case  10: /* no name, value. */
          ...
          else if (i > 0)
          {
            ...
            GtkWidget *item = gtk_menu_item_new_with_label (_(res->v[i].value));
            gtk_menu_shell_append (shell, item);
          }
          break;
