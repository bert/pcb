/*!
 * \file src/hid/gtk3/gtk3-command-window.c
 *
 * \brief Provides a command entry window for the GTK3 UI.
 *
...
command_history_add (gchar * cmd)
{
  ...
  /* Check for a duplicate command.  If found, move it to the top of the
   * list and similarly modify the combo box strings. */
  ...
  /* Not a duplicate, so put first in history list and combo box text
   * list. */
  s = g_strdup (cmd);
  history_list = g_list_prepend (history_list, s);
  gtk_combo_box_prepend_text (GTK_COMBO_BOX (ghidgui->command_combo_box), s);
...
ghid_command_use_command_window_sync (void)
{
  ...
  {
    /* Destroy the window (if it's up) which floats the
     * command_combo_box so we can pack it back into the status line
     * hbox.
     * If the window wasn't up, the command_combo_box was already
     * floating. */
    if(command_window)
      command_window_close_cb (NULL, NULL);
...
ghid_command_window_show (gboolean raise)
{
  ...
  g_signal_connect (G_OBJECT (command_window), "delete-event", G_CALLBACK (command_window_delete_event_cb), NULL);
  g_signal_connect (G_OBJECT (command_window), "key_press_event", G_CALLBACK (command_escape_cb), NULL);
  ...
  /* Make the command reference scrolled text view.
   * Use high level utility functions in gui-utils.c. */
...
ghid_command_entry_get (gchar * prompt, gchar * command)
{
   ...
   * callbacks are invokded from the resize caused by showing the combo
   * box. */
   ...
   * don't get intercepted by the menu system.
   * Set the interface insensitive so all the user can do is enter a
   * command, grab focus and connect a handler to look for the escape
   * key. */
  ...
  /* Restore the status line label and give focus back to the drawing
   * area. */
  gtk_widget_hide (ghidgui->command_combo_box);
