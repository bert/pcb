...
gboolean
gui_is_modifier_key_sym(gint ksym)
	{
...
gboolean
gui_control_is_pressed(void)
	{
	GdkModifierType		mask;

	gdk_window_get_pointer(Output.drawing_area->window, NULL, NULL, &mask);
	return (mask & GDK_CONTROL_MASK) ? TRUE : FALSE;
	}

gboolean
gui_shift_is_pressed(void)
	{
	GdkModifierType		mask;
...
gui_modifier_keys_state(void)
	...
	gdk_window_get_pointer(Output.drawing_area->window, &x, &y, &mask);
...
void
gui_draw_area_clear(OutputType *out, gboolean to_screen)
	{
...
void
gui_draw_area_update(OutputType *out, GdkRectangle *rect)
	{
	...
	if (rect)
		...
		update_rect.x = 0;
		update_rect.y = 0;
		update_rect.width = out->Width;
		update_rect.height = out->Height;
		}
	gdk_window_invalidate_rect(out->drawing_area->window, &update_rect, FALSE);
	}

void
gui_string_markup_extents(PangoFontDescription *font_desc, gchar *string,
			gint *width, gint *height)
	{
	PangoLayout *layout;
	gint        w, h;

	layout = gtk_widget_create_pango_layout(Output.top_window, NULL);
...
void
gui_draw_string_markup(GdkDrawable *drawable,
...
gchar *
gui_get_color_name(GdkColor *color)
	{
	gchar	*name;
...
void
gui_map_color_string(char *color_string, GdkColor *color)
	{
	...
	if (!color)
		return;
	if (colormap == NULL)
		colormap = gtk_widget_get_colormap(Output.top_window);
...
void
gui_button_set_text(GtkWidget *button, gchar *text)
	{
...
gboolean
gui_button_active(GtkWidget *widget)
	{
	return GTK_TOGGLE_BUTTON(widget)->active;
	}

gchar *
gui_entry_get_text(GtkWidget *entry)
	{
	gchar	*s = "";
...
void
gui_check_button_connected(GtkWidget *box, GtkWidget **button, gboolean active,
...
void
gui_button_connected(GtkWidget *box, GtkWidget **button,
			gboolean pack_start, gboolean expand, gboolean fill, gint pad,
			void (*cb_func)(), gpointer data, gchar *string)
	{
	GtkWidget   *b;
...
void
gui_spin_button(GtkWidget *box, GtkWidget **spin_button, gfloat value,
...
void
gui_table_spin_button(GtkWidget *table, gint row, gint column,
...
void
gui_range_control(GtkWidget *box, GtkWidget **scale_res, gboolean horizontal,
...
GtkWidget *
gui_scrolled_vbox(GtkWidget *box, GtkWidget **scr,
  ...
  */
GtkWidget *
gui_framed_vbox(GtkWidget *box, gchar *label, gint frame_border_width,
...
GtkWidget *
gui_framed_vbox_end(GtkWidget *box, gchar *label, gint frame_border_width,
		gboolean frame_expand, gint vbox_pad, gint vbox_border_width)
	{
	GtkWidget	*frame;
	GtkWidget	*vbox;
...
GtkWidget *
gui_category_vbox(GtkWidget *box, gchar *category_header, gint header_pad,
...
GtkTreeSelection *
gui_scrolled_selection(GtkTreeView *treeview, GtkWidget *box,
...
GtkWidget *
gui_notebook_page(GtkWidget *tabs, char *name, gint pad, gint border)
	{
	GtkWidget	*label;
	GtkWidget	*vbox;
...
GtkWidget *
gui_framed_notebook_page(GtkWidget *tabs, char *name, gint border,
			gint frame_border, gint vbox_pad, gint vbox_border)
	{
	GtkWidget	*vbox;

	vbox = gui_notebook_page(tabs, name, 0, border);
	vbox = gui_framed_vbox(vbox, NULL, frame_border, TRUE,
				vbox_pad, vbox_border);
	return vbox;
	}

void
gui_dialog_report(gchar *title, gchar *message)
	{
	GtkWidget	*top_win;
	GtkWidget	*dialog;
	GtkWidget	*scrolled;
	...
	if (nlines > 20)
		{
		vbox1 = gui_scrolled_vbox(vbox, &scrolled,
...
void
gui_label_set_markup(GtkWidget *label, gchar *text)
	{
	if (label && text)
...
void
gui_text_view_append(GtkWidget *view, gchar *string)
	{
	static gchar	*tag;
	gchar			*s;
...
void
gui_text_view_append_strings(GtkWidget *view, gchar **string,
			gint n_strings)
	{
	gchar	*tag = NULL;
	gchar	*s, *t;
...
GtkWidget *
gui_scrolled_text_view(GtkWidget *box, GtkWidget **scr,
		GtkPolicyType h_policy, GtkPolicyType v_policy)
	{
	GtkWidget	*scrolled,
				*view;
