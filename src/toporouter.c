        ...
        switch(tv->bbox->type) {
          case PIN:
            ...
            cairo_arc(dc->cr,
                tv->v.p.x * dc->s + MARGIN,
                tv->v.p.y * dc->s + MARGIN,
                (((double)pin->Thickness / 2.0f) + (double)lookup_keepaway(pin->Name) ) * dc->s, 0, 2 * M_PI);
            cairo_fill(dc->cr);
            ...
            cairo_arc(dc->cr,
                tv->v.p.x * dc->s + MARGIN,
                tv->v.p.y * dc->s + MARGIN,
                (double)(pin->Thickness) / 2.0f * dc->s,
                0, 2 * M_PI);
            cairo_fill(dc->cr);
...
epsilon_equals(double a, double b)
{
...
print_vertex(toporouter_vertex_t *v)
{
  ...
  if(v->routingedge) {
    unsigned int n = g_list_length(edge_routing(v->routingedge));
    unsigned int pos = g_list_index(edge_routing(v->routingedge), v);
...
static inline int
coord_wind(double ax, double ay, double bx, double by, double cx, double cy)
{
...
{
  double v1halfthick, v2halfthick, v1keepaway, v2keepaway, ms;
...
void
toporouter_draw_cluster(toporouter_t *r, drawing_context_t *dc, toporouter_cluster_t *cluster, double red, double green, double blue, unsigned int layer)
{
#if TOPO_OUTPUT_ENABLED
//GList *i = cluster->i;
 ...
 */
static void
coords_on_line (double ax, double ay,
                double m,
                double r,
 ...
 */
static inline double
cartesian_gradient(double x0, double y0, double x1, double y1)
{
 ...
 */
double
perpendicular_gradient(double m)
{
 ...
 */
static inline void
vertex_outside_segment(toporouter_spoint_t *a, toporouter_spoint_t *b, double r, toporouter_spoint_t *p)
{
 ...
 */
void
coord_intersect(double ax, double ay, double bx, double by, double cx, double cy, double dx, double dy, double *rx, double *ry)
{
  double ua_top, ua_bot, ua;
...
GtsVertex *
insert_vertex(toporouter_t *r, toporouter_layer_t *l, double x, double y, toporouter_bbox_t *box)
{
  GtsVertexClass *vertex_class = GTS_VERTEX_CLASS (toporouter_vertex_class ());
  GtsVertex *v;
  GList *i;
...
GList *
insert_constraint_edge(toporouter_t *r, toporouter_layer_t *l, double x1, double y1, unsigned int flags1,
    double x2, double y2, unsigned int flags2, toporouter_bbox_t *box)
{
  GtsVertexClass *vertex_class = GTS_VERTEX_CLASS (toporouter_vertex_class ());
...
void
insert_centre_point(toporouter_t *r, toporouter_layer_t *l, double x, double y)
{
  GList *i;
  GtsVertexClass *vertex_class = GTS_VERTEX_CLASS (toporouter_vertex_class ());
...
GList *
rect_with_attachments(double rad,
    double x0, double y0,
    double x1, double y1,
    double x2, double y2,
    double x3, double y3,
 ...
 */
int
read_pads(toporouter_t *r, toporouter_layer_t *l, unsigned int layer)
{
  ...
  unsigned int front = GetLayerGroupNumberBySide (TOP_SIDE);
  unsigned int back = GetLayerGroupNumberBySide (BOTTOM_SIDE);
...
triangle_cost(GtsTriangle *t, gpointer *data){
  ...
  double *min_quality = (double *)data[0];
  double *max_area = (double *)data[1];
  double quality = gts_triangle_quality(t);
  double area = gts_triangle_area(t);
...
/*
  {
    gpointer data[2];
    double quality = 0.50, area = G_MAXDOUBLE;
    unsigned int num = gts_delaunay_conform(l->surface, -1, (GtsEncroachFunc) gts_vertex_encroaches_edge, NULL);
...
toporouter_bbox_t *
toporouter_bbox_locate(toporouter_t *r, toporouter_term_t type, void *data, double x, double y, unsigned int layergroup)
{
      ...
      NET_LOOP(netlist);
      {
        ...
        CONNECTION_LOOP (net);
        {
          ...
          if(connection->type == LINE_TYPE) {
            ...
            for(unsigned int m=0;m<groupcount();m++) {
  ...
  while(hits) {
    ...
    if(box->layer == (int)z) {
      if(box->type != BOARD) {
        if(box->type == LINE) {
          LineType *line = (LineType *)box->data;
          int linewind = coord_wind(line->Point1.X, line->Point1.Y, x, y, line->Point2.X, line->Point2.Y);
...
{
  double layerpenalty = (vz(curpoint) == vz(destpoint)) ? 0. : r->viacost;
...
*/
static inline double
angle_span(double a1, double a2)
{
  if(a1 > a2)
    return ((2*M_PI)-a1 + a2);
  return a2-a1;
...
toporouter_vertex_t *
new_temp_toporoutervertex(double x, double y, toporouter_edge_t *e)
{
  GtsVertexClass *vertex_class = GTS_VERTEX_CLASS (toporouter_vertex_class ());
  GList *i = edge_routing(e);
 ...
 */
toporouter_vertex_t *
new_temp_toporoutervertex_in_segment(toporouter_edge_t *e, toporouter_vertex_t *v, double r, toporouter_vertex_t *ref)
{
  double m = edge_gradient(e);
    ...
    while(j) {
      ...
      if(!*a) {
        *a = v1; *b = v2; min = simple_h_cost(r, *a, *b);
        ...
        double tempd = simple_h_cost(r, v1, v2);
...
//  if(routedata->flags & TOPOROUTER_FLAG_FLEX) i = r->destboxes;
  ...
  while(i) {
    ...
    if(!closest) {
      ...
      double tempd = simple_h_cost(r, v, cv);
...
{
  double ic = triangle_interior_capacity(t, v);
  double flow = flow_from_edge_to_edge(t, adj_edge1, adj_edge2, v, curpoint);
...
{
  double totald, v1ms, v2ms, flow, capacity, ms;
  GList *vs = NULL;
    ...
    toporouter_vertex_t **dest, toporouter_vertex_t *curpoint)
{
  ...
  unsigned int e1intcap, e2intcap;
...
space_edge(gpointer item, gpointer data)
{
  ...
  forces = (double *)malloc(sizeof(double) * g_list_length(edge_routing(e)));

  for(unsigned int j=0;j<100;j++) {
    unsigned int k=0;
    unsigned int equilibrium = 1;
...
double
vertices_routing_conflict_cost(toporouter_t *r, toporouter_vertex_t *v, toporouter_vertex_t *pv, unsigned int *n)
{
...
double
gcost(toporouter_t *r, toporouter_route_t *data, toporouter_vertex_t *srcv, toporouter_vertex_t *v, toporouter_vertex_t *pv, unsigned int *n,
    toporouter_netlist_t *pair)
{
  double cost = 0., segcost;
...
GList *
route(toporouter_t *r, toporouter_route_t *data, unsigned int debug)
{
...
//    if(count > 100) exit(0);
    ...
    while(i) {
      ...
      if(!g_list_find(closelist, temppoint) && candidate_is_available(curpoint, temppoint)) { //&& temppoint != curpoint) {
        ...
        unsigned int temp_gn;
        double temp_g_cost = gcost(r, data, srcv, temppoint, curpoint, &temp_gn, pair);
...
double
edge_min_spacing(GList *list, toporouter_edge_t *e, toporouter_vertex_t *v, unsigned int debug)
{
  ...
  if(gts_point_distance2(GTS_POINT(origin), GTS_POINT(edge_v1(e))) < gts_point_distance2(GTS_POINT(v), GTS_POINT(edge_v1(e)))) {
    ...
    while(i) {
      ...
      if(!(nextv->flags & VERTEX_FLAG_TEMP)) {
        double ms = min_spacing(prevv, nextv);
          if(nextv == tedge_v2(e)) {
            double cms = pathvertex_arcing_through_constraint(TOPOROUTER_VERTEX(i->data), tedge_v2(e));
...
//      printf("%f ", space);
    ...
    while(i) {
      ...
      if(!(nextv->flags & VERTEX_FLAG_TEMP)) {
        double ms = min_spacing(prevv, nextv);
          if(nextv == tedge_v1(e)) {
            double cms = pathvertex_arcing_through_constraint(TOPOROUTER_VERTEX(i->data), tedge_v1(e));
...
*/
unsigned int
vertex_line_normal_intersection(double x1, double y1, double x2, double y2, double x3, double y3, double *x, double *y)
{
...
toporouter_arc_t *
toporouter_arc_new(toporouter_oproute_t *oproute, toporouter_vertex_t *v1, toporouter_vertex_t *v2, toporouter_vertex_t *centre, double r, int dir)
{
...
double
export_pcb_drawline(unsigned int layer, unsigned int x0, unsigned int y0, unsigned int x1, unsigned int y1, unsigned int thickness, unsigned int keepaway)
{
  double d = 0.;
  LineType *line;
...
double
export_pcb_drawarc(unsigned int layer, toporouter_arc_t *a, unsigned int thickness, unsigned int keepaway)
{
  double sa, da, theta;
  double d = 0.;
  ArcType *arc;
...
void
calculate_term_to_arc(toporouter_vertex_t *v, toporouter_arc_t *arc, unsigned int dir)
{
  double theta, a, b, bx, by, a0x, a0y, a1x, a1y;
  int winddir;
...
{
  double theta, a, b, bx, by, a0x, a0y, a1x, a1y, m, preva, prevb;
  int winddir;
...
{
  unsigned int layer = PCB->LayerGroups.Entries[oproute->layergroup][0];
  unsigned int thickness = lookup_thickness(oproute->style);
  unsigned int keepaway = lookup_keepaway(oproute->style);
...
line_line_distance_at_normal(
    double line1_x1, double line1_y1,
    double line1_x2, double line1_y2,
    double line2_x1, double line2_y1,
    double line2_x2, double line2_y2,
    double x, double y)
{
  double m1 = perpendicular_gradient(cartesian_gradient(line1_x1, line1_y1, line1_x2, line1_y2));
  double m2 = cartesian_gradient(line2_x1, line2_y1, line2_x2, line2_y2);
  double c1 = (isinf(m1)) ? x : y - (m1 * x);
  double c2 = (isinf(m2)) ? line2_x1 : line2_y1 - (m2 * line2_x1);
...
void
calculate_serpintine(double delta, double r, double initiala, double *a, unsigned int *nhalfcycles)
{
  double lhalfcycle = 2.*(initiala-r)+(M_PI*r);
  unsigned int n;
...
toporouter_serpintine_new(double x, double y, double x0, double y0, double x1, double y1, gpointer start, double halfa, double
    radius, unsigned int nhalfcycles)
{
...
double
check_non_intersect_vertex(double x0, double y0, double x1, double y1, toporouter_vertex_t *pathv, toporouter_vertex_t *arcv,
    toporouter_vertex_t *opv, int wind, int *arcwind, double *arcr, unsigned int debug)
{
...
double
check_intersect_vertex(double x0, double y0, double x1, double y1, toporouter_vertex_t *pathv, toporouter_vertex_t *arcv,
    toporouter_vertex_t *opv, int wind, int *arcwind, double *arcr, unsigned int debug)
{
...
new_rubberband_arc(toporouter_vertex_t *pathv, toporouter_vertex_t *arcv, double r, double d, int wind, GList *list)
{
...
/*
GtsEdge *
vertex_edge_facing_vertex(GtsVertex *v, double x, double y)
{
...
double
check_adj_pushing_vertex(toporouter_oproute_t *oproute, double x0, double y0, double x1, double y1, toporouter_vertex_t *v, double *arcr, int *arcwind, toporouter_vertex_t **arc)
{
 ...
 */
GList *
oproute_rubberband_segment(toporouter_t *r, toporouter_oproute_t *oproute, GList *path, gpointer t1, gpointer t2, unsigned int debug)
{
  double x0, y0, x1, y1;
...
//}
  ...
  if(list1) {
    ...
    double px = parc ? parc->x1 : vx(TOPOROUTER_VERTEX(t1)), py = parc ? parc->y1 : vy(TOPOROUTER_VERTEX(t1));
...
//#endif
  ...
  if(list2) {
    ...
    double nx = narc ? narc->x0 : vx(TOPOROUTER_VERTEX(t2)), ny = narc ? narc->y0 : vy(TOPOROUTER_VERTEX(t2));
...
check_speccut(toporouter_oproute_t *oproute, toporouter_vertex_t *v1, toporouter_vertex_t *v2, toporouter_edge_t *e, toporouter_edge_t *e1, toporouter_edge_t *e2)
{
  ...
  double cap, flow, line_int_x, line_int_y;
...
void
delete_route(toporouter_route_t *routedata, unsigned int destroy)
{
...
void
print_costmatrix(double *m, unsigned int n)
{
  printf("COST MATRIX:\n");
...
static inline void
init_cost_matrix(double *m, unsigned int n)
{
  for(unsigned int i=0;i<n;i++) {
    for(unsigned int j=0;j<n;j++) {
      m[(i*n)+j] = INFINITY;
...
netscore_create(toporouter_t *r, toporouter_route_t *routedata, unsigned int n, unsigned int id)
{
  ...
  netscore->pairwise_nodetour = (unsigned int *)malloc(n * sizeof(unsigned int));
...
edge_closest_vertex(toporouter_edge_t *e, toporouter_vertex_t *v)
{
  ...
  while(i) {
    toporouter_vertex_t *ev = TOPOROUTER_VERTEX(i->data);
    double tempd = gts_point_distance2(GTS_POINT(ev), GTS_POINT(v));
...
/*
double
route_conflict(toporouter_t *r, toporouter_route_t *route, unsigned int *n)
{
  GList *i = route->path;
  toporouter_vertex_t *pv = NULL;
  double cost = 0.;
...
route_checkpoint(toporouter_route_t *route, toporouter_route_t *temproute)
{
  ...
  int n = g_list_length(route->path);

  if(route->ppathindices) free(route->ppathindices);
  route->ppathindices = (int *)malloc(sizeof(int)*n);
  ...
  while(i) {
    ...
    if(v->routingedge) {
      ...
      int tempindex = g_list_index(edge_routing(v->routingedge), v);
...
{
  int oldc = routedata->dest->c, newc = routedata->src->c;
...
int
roar_route(toporouter_t *r, toporouter_route_t *routedata, int threshold)
{
  int intfails = 0;
...
int
roar_router(toporouter_t *r, int failcount, int threshold)
{
  int pfailcount = failcount +1;
...
{
  double pscore = data->score, nscore = 0.;
  GList *netlists = NULL;
...
{
  GList *i = r->routednets;
  unsigned int n = g_list_length(r->routednets);
...
int
rubix_router(toporouter_t *r, int failcount)
{
  GList *i, *ordering;
...
{
  int failcount = g_list_length(r->failednets);
...
parse_arguments(toporouter_t *r, int argc, char **argv)
{
  ...
  for(i=0;i<argc;i++) {
    if(sscanf(argv[i], "viacost=%d", &tempint)) {
      ...
      double *layer = (double *)malloc(sizeof(double));
