...
typedef struct
{
  ...
  *InitialLayerStack;		/*!< If set, the initial layer stack is set to this */
  ...
  RingBellWhenFinished,	/*!< flag if a signal should be produced when searching
                          *   if connections is done */
  AutoPlace;			/*!< flag which indicates we should force placement of the
                      *   windows on startup */
}SettingType;
