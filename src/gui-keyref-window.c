...
"<b>\tc\t",
N_("Center display at cursor location\n"),
"<b><c>\tc\t",
N_("Copy selected to buffer and unselect\n"),
"\n",

"<b>\td\t",
N_("Display pin or pad names\n"),
"<b><s>\td\t",
...
"\t",
N_("<b><ctrl>"),
N_(" - while pressed, stop drawing current line, arc, rectangle or\n"
   "\t\tpoly draws and restart the draw at the current cursor position\n"
   "\t\twith another left mouse button click.\n"),
"\n",
N_("<b>Middle button\n"),
N_("\tA click select objects if a draw operation has not been initiated.\n"),
N_("\tA press and drag moves objects.\n"),
N_("\tOr, if a line, arc, rectangle, or polygon draw operation has been\n"
   "\tinitiated, a click stops the draw operation.\n"),
"\n",
N_("<b>Right button\n"),
N_("\tRestart line, arc, rectangle, or polygon draws.  Otherwise pop\n"
   "\tup a menu.\n"),
"\n\t",
N_("<b><shift>"),
N_(" - zoom view to layout extents.\n"),
"\n",
N_("<b>Scroll wheel\n"),
N_("\tPan layout vertically.  <shift> scroll pans horizontally.\n"),
};
...
keyref_window_configure_event_cb(GtkWidget *widget, GdkEventConfigure *ev,
				gpointer data)
	{
	Settings.keyref_window_width = widget->allocation.width;
    Settings.keyref_window_height = widget->allocation.height;
...
gui_keyref_window_show(void)
	...
	gtk_window_set_default_size(GTK_WINDOW(keyref_window),
				Settings.keyref_window_width, Settings.keyref_window_height);
	...
	for (i = 0; i < sizeof(key_ref_text)/sizeof(gchar *); ++i)
		gui_text_view_append(text, _(key_ref_text[i]));
